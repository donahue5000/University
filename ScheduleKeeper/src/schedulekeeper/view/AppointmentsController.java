...
public class AppointmentsController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        timeZoneLabel.setText(ZoneId.systemDefault().toString() + " UTC" 
    ...
    //Lambda Expression to filter past appointments for removal
    //Logical Justification - cleanest way I know to filter collection
    
    @FXML
    private void deletePastAppointments(ActionEvent event) {
        List<Appointment> pastAppointments = new ArrayList(DBConnect.getRecentAppointmentList()
                .stream()
                .filter(x -> x.getStart().toInstant().isBefore(Instant.now()))
                .collect(Collectors.toList())
        );
        for (Appointment a : pastAppointments){
            DBConnect.deleteAppointment(a);
        }
    ...
    //Logical Justification - can apply multiple predicate filters in one stream
